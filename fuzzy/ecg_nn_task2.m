...
filename = sprintf('set2_train_rnn%d',k);
train1 = importdata(filename);
filename = sprintf('set2_val_rnn%d',k);
val = importdata(filename);
filename = sprintf('set2_test_rnn%d',k);
...
if traind == 2
  filename = sprintf('population2/train_rnn_%d',p);
  net1=load(filename);
  net = net1.net;
end
...
filename = sprintf('population2/train_test_val%d.txt',p);
